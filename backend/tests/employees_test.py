...
def test_add_employee(client, db_session, test_employee):
    ...
    response = client.post("/employees/employee", json=test_employee)
...
def test_update_employee(client, test_employee):
    response = client.put(f"/employees/employee/{test_employee['employeeId']}", json={
        "role": "Data Scientist",
    })
    response_json = response.json()
    assert response.status_code == 200
    ...
    get_response = client.get(f"/employees/employee/{test_employee['employeeId']}")
    assert get_response.status_code == 200
    assert get_response.json()["role"] == "Data Scientist"

def test_get_employee_by_id(client, test_employee):
    response = client.get(f"/employees/employee/{test_employee['employeeId']}")
    assert response.status_code == 200
    response_json = response.json()
...
def test_delete_employee(client, test_employee):
    response = client.delete(f"/employees/employee/{test_employee['employeeId']}")
    assert response.status_code == 200
